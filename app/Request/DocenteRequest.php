...
function Agregar(){
	...
	<form action="<?=route($_POST['model'] . '/save'); ?>" method="POST" class="form-horizontal">
		...
		<div class="form-group">
			...
			<div class="col-sm-4">
				<input type="text" name="nombre" id="nombre" value="<?=utf8_encode($docente->nombre); ?>" class="form-control" placeholder="Nombre" required>
			</div>
			<div class="col-sm-3">
				<input type="text" name="apellido_paterno" id="apellido_paterno" value="<?=utf8_encode($docente->apellido_paterno); ?>" class="form-control" placeholder="A Paterno" required>
			</div>
			<div class="col-sm-3">
				<input type="text" name="apellido_materno" id="apellido_materno" value="<?=utf8_encode($docente->apellido_materno); ?>" class="form-control" placeholder="A Materno" required>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-4">
				<input type="text" name="cedula_profesional" id="cedula" value="<?=utf8_encode($docente->cedula_profesional); ?>" class="form-control" required>
			</div>
		</div>
		<div class="form-group">
	...
	if(count($docente)>0){
		foreach ($docente as $d){ ?>
			<tr>
				...
				<td class="text-center"><i class="fa fa-check <?=($d->estatus == 'Si') ? 'text-primary' : 'text-muted'; ?>"></i></td>
				<td class="text-right">
					<button type="button" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#operationModal" data-id="<?=$d->id; ?>" data-model="Docente" data-operation="Editar">
						<i class="fa fa-edit"></i> Editar
					</button>
					<button type="button" class="btn btn-xs btn-danger" data-toggle="modal" data-target="#operationModal" data-id="<?=$d->id; ?>" data-model="Docente" data-operation="Eliminar">
